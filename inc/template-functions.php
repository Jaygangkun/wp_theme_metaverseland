 ...
 * @return array
 */
function Metaverseland_body_classes( $classes ) {
...
add_filter( 'body_class', 'Metaverseland_body_classes' );
 ...
 * @return array
 */
function Metaverseland_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'Metaverseland_post_classes', 10, 3 );
 ...
 * @return void
 */
function Metaverseland_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'Metaverseland_pingback_header' );
 ...
 * @return void
 */
function Metaverseland_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'Metaverseland_supports_js' );
 ...
 * @return array
 */
function Metaverseland_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'Metaverseland_comment_form_defaults' );
 ...
 * @return bool
 */
function Metaverseland_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'Metaverseland_can_show_post_thumbnail',
 ...
 * @return int
 */
function Metaverseland_get_avatar_size() {
	return 60;
}
 ...
 * @since Metaverseland 1.0
 */
function Metaverseland_continue_reading_text() {
 ...
 * @since Metaverseland 1.0
 */
function Metaverseland_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . Metaverseland_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'Metaverseland_continue_reading_link_excerpt' );
 ...
 * @since Metaverseland 1.0
 */
function Metaverseland_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . Metaverseland_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'Metaverseland_continue_reading_link' );

if ( ! function_exists( 'Metaverseland_post_title' ) ) {
	/**
 ...
 * @return string
 */
function Metaverseland_get_icon_svg( $group, $icon, $size = 24 ) {
	return Metaverseland_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function Metaverseland_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? Metaverseland_get_icon_svg( 'ui', 'arrow_right' ) : Metaverseland_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? Metaverseland_get_icon_svg( 'ui', 'arrow_left' ) : Metaverseland_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'Metaverseland_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function Metaverseland_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'Metaverseland_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'Metaverseland_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'Metaverseland_generate_css' ) ) {
	...
	return Metaverseland_generate_css( // @phpstan-ignore-line.
 ...
 */
function Metaverseland_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since Metaverseland 1.0
 * @since Metaverseland 1.4 Corrected parameter name for `$output`,
 ...
 */
function Metaverseland_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
...
add_filter( 'the_password_form', 'Metaverseland_password_form', 10, 2 );
 ...
 * @return array
 */
function Metaverseland_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'Metaverseland_get_attachment_image_attributes', 10, 3 );
