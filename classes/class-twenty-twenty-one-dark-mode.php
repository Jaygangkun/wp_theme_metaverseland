...
class Metaverseland_Dark_Mode {
	...
	public function editor_custom_color_variables() {
		...
		if ( $should_respect_color_scheme && Metaverseland_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
	...
	public function customizer_controls( $wp_customize ) {
		...
		$wp_customize->add_control(
			new Metaverseland_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => function() {
						return 127 >= Metaverseland_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: Metaverseland support article URL. */
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= Metaverseland_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= Metaverseland_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'Metaverseland uses LocalStorage when Dark Mode support is enabled.', 'twentytwentyone' ) . '</p>'
		...
		wp_add_privacy_policy_content( 'Metaverseland', wp_kses_post( wpautop( $content, false ) ) );
	}
